...
function App() {
    ...
    try {
      ...
      const data = rows.map((row: any) => {
        ...
        row.c.forEach((cell: any, i: number) => {
          ...
          if (!colName) return;
          // ROBUST LINK EXTRACTION: handles both plain URLs and HYPERLINK formulas
  ...
  // FALLBACK BRIEFING: always works, even if Gemini API fails
...
${JSON.stringify(filtered.slice(0, 15), null, 2)}`;
    ...
    try {
      ...
      if (data.candidates?.[0]?.content?.parts?.[0]?.text) {
        ...
        // API error — use fallback so button still works
        setBriefing(generateFallbackBriefing(filtered));
      } else {
        setBriefing(generateFallbackBriefing(filtered));
      }
    } catch (err) {
      // Network error — use fallback so button still works
      setBriefing(generateFallbackBriefing(filtered));
    }
